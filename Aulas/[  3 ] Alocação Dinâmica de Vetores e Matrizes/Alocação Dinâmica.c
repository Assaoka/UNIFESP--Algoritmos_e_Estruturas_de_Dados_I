...
.       Vetores são estruturas de dados que armazenam um conjunto de valores do mesmo tipo. Para a linguagem C, um vetor é um ponteiro para o primeiro elemento do conjunto de valores. A partir do endereço do primeiro elemento, é possível acessar qualquer outro elemento com o uso de aritmética de ponteiros.
.       Podemos receber um vetor já existente e acessar seus elementos da seguinte forma:*/
int v[10], *p, i;
p = v; // ou p = &v[0];
...
.       Esses dois operadores podem ser combinados de diversas formas para operar sobre vetores. Vamos analisar cada uma das expressões, considerando que p aponta para o primeiro elemento de um vetor v.
...
Free:
.       Essa função recebe como parâmetro um ponteiro para um bloco de memória alocado e libera esse bloco de memória. 
.       Essa função é muito importante para evitar vazamento de memória, pois o espaço alocado dinamicamente não é liberado automaticamente pelo sistema operacional.
.       Após a chamada da função, o ponteiro passa a apontar para um endereço inválido, então é uma boa prática atribuir NULL ao ponteiro após a chamada da função.*/
int *p;
p = (int *) malloc(10 * sizeof(int)); // aloca um vetor de 10 inteiros
...
.       Para alocar uma matriz, vamos utilizar um vetor de ponteiros para o tipo de dado desejado. E cada posição desse vetor vai apontar para um vetor de elementos do tipo desejado. Caso a matriz tenha mais dimensões, basta repetir o processo para cada dimensão.*/
int **m, i;
